...
ftable_regions(d::star) = [r for r in 1:(7 * d.P + 4) if mod1(r, 7) != 7]
...
"""
    boundquad!(P::SubArray{Float64}, d::abstractdomain, k::Int) -> nothing

Find a bounding quadrilateral for the k-th patch.

Uses `ftable_regions(d)` as the list of non-rectangular regions. If `p.reg`
is not in `ftable_regions(d)`, the patch is treated as affine/rectangular.
"""
#|| always requires the left operand to be a Bool
#|| can return non-boolean values.
#false || "ok"    # returns "ok"
#true || "nope"   # returns true
function boundquad!(P::SubArray{Float64}, d::abstractdomain, k::Int)

   p = d.pths[k]

   # Four corners.
   L1p1x, L1p1y = mapm1(d, -1.0, k)
   L1p2x, L1p2y = mapp1(d, -1.0, k)
   L2p1x, L2p1y = mapm1(d, 1.0, k)
   L2p2x, L2p2y = mapp1(d, 1.0, k)

   # If region is not in ftable_regions(d), it is rectangular/affine.
   if !(p.reg in ftable_regions(d))
   ...
   @inbounds for (i, t) in enumerate(tpts)
...
"""
    plot_dfunc(d::abstractdomain; L::Int=65, cmap=:viridis,
               linewidth::Float64=1.0, show::Bool=true)

Plot the pure distance factor `dfunc` over all patches of the domain `d`.

For each patch `k`, this plots the scalar field

    ρ_k(u,v) = dfunc(d, k, 1 - u)

pulled forward to physical space by the patch map `τ_k`.

Notes
-----
- This uses the convention already built into your `dfunc!` functions:
  the input must be `1 - u_actual`, where `u = 1` is the boundary side.
- Rectangular / affine patches will automatically show the correct constant
  value (typically `1`).
- Requires:
    mapxy!(Zx, Zy, d, U, V, k)
    dfunc!(D, d, k, T)
"""
function plot_dfunc(d::abstractdomain; L::Int=65, cmap=:viridis,
   linewidth::Float64=1.0, show::Bool=true)

   fig = Figure(size=(900, 900))
   ax = Axis(fig[1, 1], aspect=DataAspect())

   u = collect(range(-1.0, 1.0; length=L))
   v = collect(range(-1.0, 1.0; length=L))

   U  = Matrix{Float64}(undef, L, L)
   V  = Matrix{Float64}(undef, L, L)
   T  = Matrix{Float64}(undef, L, L)
   Zx = Matrix{Float64}(undef, L, L)
   Zy = Matrix{Float64}(undef, L, L)
   D  = Matrix{Float64}(undef, L, L)
   ...
   @inbounds for k in 1:d.Npat
      ...
      dmin = min(dmin, localmin)
      dmax = max(dmax, localmax)
   end
   ...
   @inbounds for k in 1:d.Npat
      ...
      # Important: copy arrays before passing to Makie.
      # Otherwise all surfaces may reference the same mutated arrays.
      Xp = copy(Zx)
      Yp = copy(Zy)
      Dp = copy(D)
      Zp = zeros(Float64, L, L)

      plt = surface!(ax, Xp, Yp, Zp;
         color=Dp,
         colormap=cmap,
         colorrange=(dmin, dmax),
         shading=NoShading)

      lines!(ax, copy(Xp[:, 1]), copy(Yp[:, 1]),
         color=:black, linewidth=linewidth)

      lines!(ax, copy(Xp[:, L]), copy(Yp[:, L]),
         color=:black, linewidth=linewidth)
   ...
   #Colorbar(fig[1, 2], plt, label="distance factor")
   #Colorbar(fig[1, 2], plt)
   Colorbar(fig[1, 2], plt;
    label = "",                         # remove "distance factor"
    ticks = ([0.0, 1.0], ["0.0", "1.0"]),  # show only 0.0 and 1.0
    ticklabelsize = 22,                 # increase font size
   )
...
"""
    curvature_from_DLP(d, t, k)

Return the signed curvature at boundary coordinate `t` on boundary patch `k`,
using the diagonal DLP value.

Assumes the convention

    DLP(d,t,t,k) = -0.5 * κ(t) * |γ'(t)|

where γ(t) is the local boundary parametrization.
"""
function curvature_from_DLP(d::abstractdomain, t::Float64, k::Int)::Float64

   dx, dy = dgam(d, t, k)

   return -2.0 * DLP(d, t, t, k) / hypot(dx, dy)
end
